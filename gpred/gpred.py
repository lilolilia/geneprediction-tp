...
def find_start(
    ...
    if match_object:
        position = start + match_object.start()
        return position
    return None
...
def has_shine_dalgarno(
    ...
    stop_dal = start - 6
    start_dal = start - max_shine_dalgarno_distance
    if start_dal > 0:
        match_object = shine_regex.search(sequence, start_dal, stop_dal)
        if match_object:
            return True
    return False
            ...
            if stop:
                if ((stop) - cur_pos) >= min_gene_len:
                    if has_shine_dalgarno(
                        shine_regex, sequence, cur_pos, max_shine_dalgarno_distance
                    ):
                        gene_pos.append([cur_pos + 1, stop + 3])
...
def main() -> None:  # pragma: no cover
    ...
    predicted_genes_rc = predict_genes(
        sequence_rc,
        start_regex,
        stop_regex,
        shine_regex,
    ...
    probable_genes_comp = [
        ...
        for gene in predicted_genes_rc
    ]

    # Call to output functions
    # write_genes_pos(args.predicted_genes_file, probable_genes.extend(probable_genes_comp))
    write_genes(
        args.fasta_file, sequence, predicted_genes, sequence_rc, probable_genes_comp
    )
    write_genes_pos(args.predicted_genes_file, predicted_genes + probable_genes_comp)
